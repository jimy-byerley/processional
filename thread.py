...
class Thread(threading.Thread):
	''' a thread object that handles exceptions better than `threading.Thread`
	
		Args:
			target (callable):  the callable the thread will execute
			daemon (bool):      if True, the thread will automatically exit with the main thread
			name (str):         a name for the thread (optional, just for debug purpose)
			warnignore (bool):  if True, each ignored interruption will issue a warning
			warnerror (bool):   if True, a fatal exception in the thread will be printedout (event if handled afterward)
	'''
	def __init__(self, target, daemon=None, name=None, warnignore=False, warnerror=False):
	...
	def run(self):
		...
		try:
			...
			self.error = err
			self.report = traceback.format_tb()
		else:
			self.error = None
	...
	class NoInterrupt(object):
		''' context manager for locking interruption of a thread '''
...
class SlaveThread:
	...
	def close(self) -> 'ThreadTask':
		''' stop the thread reception loop. 
			It will stop once finished the already scheduled tasks 
		'''
		return ThreadTask(self, None)
		
	def schedule(self, func) -> 'ThreadTask':
	...
	def step(self) -> bool:
		...
		while self.tasks:
			...
			if id in self.register:
				self.register[id] = (error, result, report)
			elif error:
				print('Exception in', self)
				print(report)
	...
	def abort(self, exception=None):
		...
		while self.tasks:
			id, task = self.tasks.popleft()
			self.register[id] = (exception, None, None)
		with self.recvsig:
			self.recvsig.notify_all()
		self.busy = False
